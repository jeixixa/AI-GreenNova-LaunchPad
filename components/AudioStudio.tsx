...
import { Loader2, Mic, Play, Pause, Bookmark, Check, User, Upload, X, Plus, FileAudio, Sliders, Radio, Square, Trash2, FileText, Type, Save } from 'lucide-react';
...
const AudioStudio: React.FC = () => {
  ...
  // Initialize Audio Context and Load Auto-Saved Data
  useEffect(() => {
    const ctx = new (window.AudioContext || (window as any).webkitAudioContext)({sampleRate: 24000});
    setAudioContext(ctx);
    
    const savedData = localStorage.getItem('sbl_autosave_audio_studio_v4');
    if (savedData) {
        try {
            const parsed = JSON.parse(savedData);
            if (parsed.ttsScript) setTtsScript(parsed.ttsScript);
            ...
            if (parsed.rawAudioBase64) {
                const binaryString = atob(parsed.rawAudioBase64);
                const bytes = new Uint8Array(binaryString.length);
                for (let i = 0; i < binaryString.length; i++) {
                    bytes[i] = binaryString.charCodeAt(i);
                }
                setRawAudio(bytes);
                decodeAudioData(bytes, ctx).then(setAudioBuffer).catch(console.error);
            }
            if (parsed.lastSaved) setLastSaved(parsed.lastSaved);
        } catch(e) {
            console.error("Failed to restore Audio Studio session", e);
        }
    }
  ...
  useEffect(() => {
      ...
      const data: any = { ttsScript, transcript, selectedVoice, mode, activeTab, lastSaved: timestamp };
      
      // Persist raw audio if it's small enough (avoid localstorage limits)
      if (rawAudio && rawAudio.length < 3000000) {
          let binary = '';
          const len = rawAudio.byteLength;
          for (let i = 0; i < len; i++) {
              binary += String.fromCharCode(rawAudio[i]);
          }
          data.rawAudioBase64 = btoa(binary);
      }
      
      const timer = setTimeout(() => {
        localStorage.setItem('sbl_autosave_audio_studio_v4', JSON.stringify(data));
        setLastSaved(timestamp);
      }, 1000); // Debounce saves

      return () => clearTimeout(timer);
  }, [ttsScript, transcript, selectedVoice, mode, rawAudio, activeTab]);
  ...
  const handleGenerate = async () => {
    if (!ttsScript || !audioContext) return;
    ...
    try {
      ...
      const base64Audio = await generateSpeech(ttsScript, voiceToUse);
  ...
  const handleSave = () => {
    ...
    try {
        ...
        reader.onloadend = () => {
            ...
            const title = mode === 'tts' && ttsScript ? `Audio (${selectedVoiceName}): ${ttsScript.substring(0, 20)}...` : `Transcribed Audio ${new Date().toLocaleTimeString()}`;
    ...
    <div className="max-w-7xl mx-auto h-full flex flex-col animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 flex-1">
        ...
        <div className="lg:col-span-8 space-y-6">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-6 flex flex-col h-full max-h-[600px] relative">
                {mode === 'tts' ? (
                    <>
                        <div className="flex justify-between items-center mb-4">
                            <label className="text-sm font-bold text-gray-900 dark:text-white">Script</label>
                            <span className="text-xs text-gray-400">{ttsScript.length} chars</span>
                        </div>
                        <textarea 
                            value={ttsScript}
                        ...
                        <div className="mt-6 flex justify-between items-center">
                            <div className="flex items-center gap-1.5 text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                                {lastSaved && <><Save size={12} className="text-gray-500" /> Auto-saved {lastSaved}</>}
                            </div>
                            <button
                                onClick={handleGenerate}
                                disabled={loading || !ttsScript}
                                className={`
                ...
                <div className="bg-gray-900 text-white p-6 rounded-xl flex flex-col sm:flex-row items-center justify-between shadow-xl border border-gray-800">
                    <div className="flex items-center space-x-5 mb-4 sm:mb-0 w-full sm:w-auto">
                        ...
                        <div className="min-w-0 flex-1">
                            <h3 className="font-bold text-white text-lg truncate">{mode === 'tts' && ttsScript ? 'Generated Output' : 'Audio Playback'}</h3>
                            <div className="flex items-center text-xs text-gray-400 gap-3">
                                <span className="flex items-center"><User className="w-3 h-3 mr-1" /> {mode === 'tts' ? (voices.find(v => v.id === selectedVoice)?.name || selectedVoice) : 'Uploaded File'}</span>
