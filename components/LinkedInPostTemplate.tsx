...
import { Copy, Check, ThumbsUp, MessageSquare, Repeat2, Send, Globe, MoreHorizontal, Linkedin } from 'lucide-react';
...
#Entrepreneurship #Productivity #AI #BusinessGrowth #SystemsThinking`;
  ...
  const handleCopy = () => {
    ...
    window.dispatchEvent(new CustomEvent('sbl-toast', { detail: { message: 'LinkedIn post copied', type: 'success' } }));
  };

  const handleShareToLinkedIn = () => {
    navigator.clipboard.writeText(content);
    window.dispatchEvent(new CustomEvent('sbl-toast', { 
        detail: { message: 'Post copied! Opening LinkedIn...', type: 'info', title: 'READY TO SHARE' } 
    }));
    // Open LinkedIn in a new tab. shareActive=true helps some users go straight to the composer.
    ...
    <div className="max-w-2xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-6 bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
            ...
            <div className="flex gap-2">
                <button 
                    onClick={handleCopy}
                    className="flex items-center gap-2 px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-full font-bold text-sm hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors"
                >
                ...
                <button 
                    ...
                    className="flex items-center gap-2 px-5 py-2 bg-[#0a66c2] text-white rounded-full font-bold text-sm hover:bg-[#004182] transition-colors shadow-lg"
                >
